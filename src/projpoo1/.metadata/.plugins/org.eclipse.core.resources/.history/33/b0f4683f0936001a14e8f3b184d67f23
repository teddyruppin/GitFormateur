...
public class ProjetSpringBoot01Application implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		for(Partie partie:servicePCrud.findAll()) {
			System.out.println(" parties " + partie.getId() + " joueur id " + partie.getJoueur());
			
		}
		
		for(Question question:serviceQCrud.findAll()) {
			System.out.println(" questions " + question.getId());
			
		}
		
		for(Verbe verbe:serviceVCrud.findAll()) {
			System.out.println(" verbes " + verbe.getId());
