...
public class CreerEleveController {
	...
	public String afficher(final ModelMap pModel) {
		...
		if (pModel.get("creation") == null) {
			pModel.addAttribute("creation", new CreationFormEleve());
		}
		return "creation";
	}
	
	@RequestMapping(value = "/creerCreationEleve", method = RequestMethod.POST)
	public String creer(@Valid @ModelAttribute(value = "creation") final CreationFormEleve pCreation,
		...
		if (!pBindingResult.hasErrors()) {
			...
			sd.setLenient(false); 
			Date date = new Date(); 
			try { date = sd.parse(pCreation.getDate());
			 }catch(Exception e) { 
			e.getStackTrace(); } 
